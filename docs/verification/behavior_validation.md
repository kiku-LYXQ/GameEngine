...
1. `pytest server/agents/tests/test_behavior_spec.py`（确保每个 archetype 都能生成含完整字段的 payload，并验证无效 LLM 输出会回退至默认）。将 pytest 输出复制到 `archives/behavior_validation/behavior_spec_pytest.txt` 以作为验收证据。
2. `curl http://127.0.0.1:7000/agents/behavior-spec/character?use_llm=false`（验证 API 在关闭 LLM 的情况下仍然返回合法 schema，并将响应保存到 `archives/behavior_validation/behavior_spec_api.json`）。如果需要 LLM 输出，可重试加上 `?use_llm=true` 并记录命令与返回。
...
`archives/behavior_validation/` 汇总了验证过程的产物（禁止覆盖该目录中的现有存档——由 coder3 管理）。我们关注的子目录如下：
...
3. `runtime_metrics/`：存放 `llm_metrics.json`（LLM 端点）与 `agent_health_metrics.json`（agent 健康指标）。`scripts/verify_behavior.sh` 会把收集到的 JSON 同时写入文件并打印到标准输出以便流水线读取。
...
## 验证脚本与实操提示
- `scripts/verify_behavior.sh behavior_spec_example.json` 会执行完整的校验（行为结构、资源绑定、代码有效性、运行时指标、发布说明），并按照上述目录结构生成文件。脚本期望 `agent` 与 `llm_runtime` 服务在 `127.0.0.1:7000/7001` 上运行。
