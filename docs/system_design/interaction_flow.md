...
- `scripts/verify_all.sh` 可在确认服务启动后依次运行 `validate_copilot.sh`, `validate_npc.sh`, 并输出最新 metrics，`docs/getting_started.md` 里也指明，可作为 GitHub Actions CI 验证（see `.github/workflows/verify.yml`）。
```bash
